...
public interface ControlledRunnableListener {
  ...
  /**
   * This listener method is called when the state of the {@link ControlledRunnable} changes. The
   * state is indicated by the {@link StateType} value, which can be obtained with
   * {@link ControlledRunnableEvent#getStateType()}.<br />
   * <br />
   * The actual state has to be checked on the {@link ControlledRunnable} which caused the event.
   * Here is an example which checks all the states:<br />
   * 
   * <pre>
   * 
   ...
   *       // "Will stop" has to be tested before the running state. It is still running when the
   *       // signal
   *       // to stop comes in.
   *       state = "will stop";
